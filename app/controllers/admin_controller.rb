class AdminController < ApplicationController
  ...
  def index
    ...
    @orders_by_day =
      Order.where("created_at > ?", Time.now - 7.days).order(:created_at)
    @orders_by_day =
      @orders_by_day.group_by { |order| order.created_at.to_date }
    @revenue_by_day =
      @orders_by_day.map do |day, orders|
    ...
    if @revenue_by_day.count < 7
      ...
      ordered_days_with_current_last =
        days_of_week[next_day_index..-1] + days_of_week[0...next_day_index]

      complete_ordered_array_with_current_last =
        ordered_days_with_current_last.map do |day|
